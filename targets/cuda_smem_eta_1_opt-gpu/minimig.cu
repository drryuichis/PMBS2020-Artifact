...
__global__ void target_pml_3d_kernel(
    ...
    if (threadIdx.z < 6) {
        const int z = threadIdx.z & 1;
        const int z9 = z*9;
        const int zz = (z * (N_THREADS_PER_BLOCK_DIM+1)) - 1;

        const bool xzswap = threadIdx.z <= 1;
        const bool yzswap = (threadIdx.z & 2) == 2;

        const int si = xzswap ? z9 : (threadIdx.x+1);
        const int sj = yzswap ? z9 : (threadIdx.y+1);
        const int sk = xzswap ? (threadIdx.x+1) : (yzswap ? (threadIdx.y+1) : z9);

        const int gi = xzswap ? zz : threadIdx.x;
        const int gj = yzswap ? zz : threadIdx.y;
        const int gk = xzswap ? threadIdx.x : (yzswap ? threadIdx.y : zz);
        const llint gidx = IDX3_eta1(i0+(llint)gi, j0+(llint)gj, k0+(llint)gk);

        s_eta[si * S_ETA_DIM2 + sj * S_ETA_DIM + sk] = eta[gidx];
    }
